...
import { getClawHubHomeStructuredData } from "../lib/homeStructuredData";
...
function RootDocument({ children }: { children: React.ReactNode }) {
  ...
  const showClawHubStructuredData = getSiteMode() === "skills" && location.pathname === "/";
  const clawHubStructuredData = showClawHubStructuredData
    ? JSON.stringify(getClawHubHomeStructuredData()).replace(/</g, "\\u003c")
    : null;
  ...
  return (
    <html lang="en">
      <head>
        ...
        />
        {clawHubStructuredData ? (
          <script
            type="application/ld+json"
            dangerouslySetInnerHTML={{ __html: clawHubStructuredData }}
          />
        ) : null}
